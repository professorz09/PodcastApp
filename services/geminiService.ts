...
export const generatePhoneStudioScript = async (
  ...
  const ytDynamicInstructions = isYtClaims
    ? (isHindi
        ? `\n⚠️ CONVERSATION OPENING (pehle 2 turns):\n- ${speakers[0] ?? 'Speaker 1'} se shuru karo — aise jaise dono ne ye video abhi dekha ho. Video ka naam/host mention karo agar pata ho. Bolo ki kuch claims interesting/controversial lage. Template mat use karo — natural rakhna.\n- Example: "Yaar, tune ye XYZ ka episode dekha? Usme kuch cheezein boli gayi hain jo mujhe samajh nahi aayi..." ya "Bhai, is podcast mein unhone jo bola — ek baar sun..."\n- ${speakers[1] ?? 'Speaker 2'} naturally respond kare aur pehle claim ki taraf move ho.\n\n⚠️ CLAIM DISCUSSION RULES:\n- हर claim को specifically discuss करो — generic mat bolo.\n- Claim quote करो, react करो, phir context/facts/science bolo.\n- Alag alag claims par alag dynamics rakhna — kabhi ek defend kare doosra destroy kare, kabhi dono skeptical, kabhi shocked + explain.\n- Roles swap karte raho — kabhi ${speakers[0] ?? 'Speaker 1'} defend kare, kabhi ${speakers[1] ?? 'Speaker 2'}.\n- CONCLUSION (last 2 turns): Dono apna personal verdict den — "mere hisaab se is video ka overall..." aur apni rating kyun.`
        : `\n⚠️ CONVERSATION OPENING (first 1-2 turns — very important):\nBoth speakers have ALREADY watched this podcast/video. The audience has seen it too. This is NOT a discovery — it's a reaction/discussion that happens AFTER watching.\n\n- ${speakers[0] ?? 'Speaker 1'} opens as if calling ${speakers[1] ?? 'Speaker 2'} right after finishing the episode. Use the show name and host/guest names from context above.\n- Do NOT say "I found a video", "there's a video going around", "I came across", or anything that implies just discovering it now.\n- The opening should feel like a callback — both already know what was said. One brings up a specific claim that stuck with them.\n- Good opening examples (adapt with real show/host names from context):\n  "Okay so that [Show Name] episode with [Guest] — that one claim about X is still messing with me."\n  "Hey ${speakers[1] ?? 'Speaker 2'}, I just finished that [Host] podcast. Bro, the part where he said [claim] — what do you even make of that?"\n  "Dude that [Show] episode. I've been thinking about what [Host/Guest] said about [topic] ever since."\n- ${speakers[1] ?? 'Speaker 2'} responds like they also watched it: "Yeah I saw that — the [claim] part especially..." and they dive straight into discussion.\n- NEVER mention "video circulating", "found online", or act like they're introducing the video to each other.\n\n⚠️ CLAIM DISCUSSION RULES (follow exactly):\n\nSTYLE-SPECIFIC DYNAMICS FOR EACH CLAIM:\n${ytClaimDynamics[phoneConvoStyle]}\n\nGENERAL RULES:\n- Reference each claim specifically — do NOT speak generically about the topic.\n- Quote or paraphrase the claim, react to it, then dig into the real facts/science/context.\n- Vary the dynamic on each claim — do NOT repeat the same pattern twice in a row.\n- Rotate roles — sometimes ${speakers[0] ?? 'Speaker 1'} leads/defends, sometimes ${speakers[1] ?? 'Speaker 2'} does.\n\nCONCLUSION (MANDATORY — last 2 turns of the script):\n- ${speakers[0] ?? 'Speaker 1'}: Give your personal overall verdict on this video — what held up, what didn't, one sentence rating (e.g. "6/10 — the chemistry angle was solid but the conspiracy framing was a stretch").\n- ${speakers[1] ?? 'Speaker 2'}: Give YOUR verdict — it must be DIFFERENT from the first speaker's. Harsher, more forgiving, or focused on a different angle.\n- Conclusions must feel personal and specific — not generic wrap-up lines.`)
    : '';
