...
function patchPositionSettings(filepath) {
  let code = fs.readFileSync(filepath, 'utf8');

  const oldCode = `                        {/* Position (X, Y) */}
  ...
  const newCode = `                        {/* Position (X, Y) */}
                        <div className="space-y-2">
                          ...
                          {/* Controls Disabled if Intro */}
                          <div className={\`transition-opacity \${isCurrentSegmentIntro ? 'opacity-30 pointer-events-none grayscale' : 'opacity-100'}\`}>
  ...
  if (code.includes(oldCode)) {
    ...
    console.log("Patched UI position settings in " + filepath);
  } else {
    console.log("Old code not found in " + filepath);
  }
}

patchPositionSettings('components/EnglishVideoMaker.tsx');
