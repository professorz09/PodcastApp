...
//
// NOTE: this intentionally duplicates services/vertexProxy.ts instead of
// importing it. Vercel's Node function builder does not reliably bundle/trace
// relative imports of local .ts siblings outside api/ for this project (seen
// in production as ERR_MODULE_NOT_FOUND for services/vertexProxy at runtime,
// both with and without an explicit .ts extension on the import) — every
// deployed function must be self-contained. server.ts (local dev via tsx)
// still imports the shared services/vertexProxy.ts module directly.
...
function isValidPrivateKey(keyStr: any): boolean {
...
function getGeminiClient(): {
  ...
  if (saKey) {
    ...
    if (parsed.valid && parsed.credentials && parsed.projectId) {
      ...
      console.warn(`[Gemini] GCP_SA_KEY parse issue: ${vertexParseError}`);
    }
  }
