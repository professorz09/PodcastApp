...
// Loads an image URL (data: URL or remote) and draws it cover-fit into a
// W×H canvas — same treatment as captureVideoFrame, but for an AI-generated
// storyboard illustration instead of a real video frame.
const loadImageIntoCanvas = (url: string, W: number, H: number): Promise<HTMLCanvasElement> =>
  new Promise((resolve, reject) => {
    const img = new Image();
    ...
    img.onerror = () => reject(new Error('Storyboard image load failed'));
    img.src = url;
  });
...
const IntroFlow: React.FC<IntroFlowProps> = ({ segments, podcastTitle, podcastHost, podcastGuests, selectedRanges, selectionLabel, onBlobReady, buttonOnly, videoFile }) => {
  ...
  const generateOneScene = async (idx: number) => {
    ...
    try {
      ...
      toast.error(e?.message || `Scene #${idx + 1} image fail hui`);
    } finally {
  ...
  const uploadSceneImage = (idx: number, file: File) => {
    const reader = new FileReader();
    reader.onload = (ev) => {
      const result = ev.target?.result as string;
      const scenes = introScenesRef.current;
      if (!scenes?.[idx]) return;
      scenes[idx] = { ...scenes[idx], imageUrl: result };
      introScenesRef.current = [...scenes];
      setIntroScenes([...scenes]);
    };
    reader.readAsDataURL(file);
    ...
    try {
      ...
      if (startIdx <= 3) {
        ...
        try {
          ...
          // inside where the clip starts) instead of an illustrated scene.
          // Falls back to an AI storyboard illustration if capture fails or
          // no video was uploaded — and to the synthetic phone mockup only
          // if that illustration generation also fails.
          ...
          if (videoFile) {
            ...
            try {
              ...
              console.warn('[IntroFlow] footage frame capture failed, falling back to storyboard illustration', e);
            }
          }
  ...
  if (buttonOnly) {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        ...
        {/* Intro scenes — same two-step system as EnglishVideoMaker: first
            "Plan Scenes" breaks the intro narration into timed scene-beat
            prompts (no images yet), then each scene gets its own thumbnail
            that can be AI-generated, regenerated, or replaced with an
            uploaded image — plus a "Generate All Visuals" shortcut. Only
            used when no real footage video is uploaded (real footage always
            wins in the render step), but stays visible either way so it can
            be prepared ahead of time. */}
        {videoFile && (
          <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.35)' }}>Footage upload hai — scenes use nahi honge.</div>
        )}
          ...
          <div style={{ borderRadius: 12, border: '1px solid rgba(255,255,255,0.08)', background: '#0a0a0a', overflow: 'hidden' }}>
            <div style={{ padding: '8px 10px', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(255,255,255,0.02)' }}>
              <span style={{ fontSize: 10, fontWeight: 700, color: '#c4b5fd', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                Timeline · {introScenes.length} scenes
              </span>
              <button onClick={clearScenes} style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.4)', fontSize: 9, fontWeight: 700, textTransform: 'uppercase', cursor: 'pointer' }}>Clear</button>
            </div>
            <div>
              {introScenes.map((scene, idx) => (
                <div key={idx} style={{ padding: 10, display: 'flex', gap: 8, borderBottom: idx < introScenes.length - 1 ? '1px solid rgba(255,255,255,0.05)' : 'none' }}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 5, flexShrink: 0, width: 76 }}>
                    <div style={{ position: 'relative', width: 76, height: 44, borderRadius: 8, overflow: 'hidden', background: '#111', border: '1px solid rgba(255,255,255,0.1)' }}>
                      <input
                        type="file"
                        accept="image/*"
                        onChange={e => { const f = e.target.files?.[0]; if (f) uploadSceneImage(idx, f); e.target.value = ''; }}
                        style={{ position: 'absolute', inset: 0, opacity: 0, cursor: 'pointer', zIndex: 1 }}
                        title="Upload custom image"
                      />
                      {sceneLoading[idx] ? (
                    ...
                    <button
                      ...
                      style={{
                        ...
                        background: 'rgba(255,255,255,0.04)', color: 'rgba(255,255,255,0.7)',
                        fontSize: 9, fontWeight: 600, fontFamily: 'inherit', cursor: 'pointer',
                        opacity: sceneLoading[idx] ? 0.4 : 1,
                      }}
                    >
                      <RefreshCw size={9} style={sceneLoading[idx] ? { animation: 'spin 1s linear infinite' } : undefined} />
                      {scene.imageUrl ? 'Regen' : 'Generate'}
                    </button>
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 9, color: '#c4b5fd', fontFamily: 'monospace', fontWeight: 700, marginBottom: 4 }}>
                      Scene #{idx + 1} · {scene.startOffset.toFixed(1)}s → {scene.endOffset.toFixed(1)}s
                    </div>
...
// ─── TitleThumbComboCard — title/thumbnail-text options + thumbnail image ─────
// Generated from whatever text it's given (post-generation: the actual
// script + the source clip's transcript, so it reflects what was really
// made rather than guessing off the raw full-episode transcript upfront).
interface TitleThumbComboCardProps {
  srcText: string;
  topicName: string;
}

const TitleThumbComboCard: React.FC<TitleThumbComboCardProps> = ({ srcText, topicName }) => {
  ...
  const handleGenerateCombo = async () => {
    if (!srcText.trim()) { setComboError('Script/transcript empty hai'); return; }
    setComboLoading(true);
    setComboError(null);
    try {
      const result = await generateTitleTextPair(srcText.slice(0, 3500));
  ...
  return (
    <div style={{ borderRadius: 12, border: '1px solid rgba(234,179,8,0.25)', background: 'rgba(234,179,8,0.05)', padding: 10, display: 'flex', flexDirection: 'column', gap: 8 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ fontSize: 10, fontWeight: 800, color: '#fde68a', letterSpacing: '0.04em', textTransform: 'uppercase' }}>⚡ Title + Thumbnail</div>
        <button
          onClick={handleGenerateCombo}
          disabled={comboLoading || !srcText.trim()}
          style={{
      ...
      {selectedCombo && (
        <div style={{ borderTop: '1px solid rgba(234,179,8,0.15)', paddingTop: 8, display: 'flex', flexDirection: 'column', gap: 8 }}>
          {thumbUrl ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      ...
      {phase === 'ready' && (
        <>
          ...
          {/* Title + Thumbnail now lives in the Export tab, once the actual
              script exists — see the note below and TitleThumbComboCard. */}
          <div style={{ padding: '10px 12px', borderRadius: 10, background: 'rgba(234,179,8,0.06)', border: '1px solid rgba(234,179,8,0.2)', fontSize: 10, color: 'rgba(255,255,255,0.4)', lineHeight: 1.5 }}>
            ⚡ Title + Thumbnail Export tab mein milega — Generate ke baad, actual script se banega.
          </div>
...
const PhoneConvoStudio2: React.FC<Props> = ({ mainScript, sourceClips: sourceClipsProp, videoFile: videoFileProp, embedded = false, onGeneratorComplete }) => {
  ...
  // One shared character guide for intro + all segment storyboards in this project.
  const studioCharacterGuideRef = useRef('');
  ...
  const [podcastSegments, setPodcastSegments] = useState<PodcastTranscriptSeg[]>([]);
  const [podcastTitle, setPodcastTitle] = useState('');
  const [podcastHost, setPodcastHost] = useState('');
  const [podcastGuests, setPodcastGuests] = useState<string[]>([]);
  ...
  useEffect(() => {
    ...
    const turns: ScriptTurn[] = mainScript.map(seg => {
      ...
      return {
        ...
        ...(isIntro && seg.learnEnglish?.introScenes?.length && { introScenes: seg.learnEnglish.introScenes }),
      };
    });
    setScript(turns);
    ...
    loadPhoneStudio2Scenes(mainScript).then(loaded => {
      if (!loaded) return;
      if (loaded.introCharacterGuide) studioCharacterGuideRef.current = loaded.introCharacterGuide;
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      savePhoneStudio2Scenes(
        mainScript,
        script,
        studioCharacterGuideRef.current,
      );
    }, 600);
  ...
  const planTurnScenes = async (turn: ScriptTurn, field: 'introScenes' | 'segmentScenes') => {
    ...
    try {
      ...
      const existingGuide = studioCharacterGuideRef.current;
      const { prompts, usesCharacter, characterGuide } = await generateStoryboardScenesTimeBased(slots, 'gemini-3.5-flash', existingGuide || undefined);
      if (characterGuide) studioCharacterGuideRef.current = characterGuide;
      else if (existingGuide) studioCharacterGuideRef.current = existingGuide;
  ...
  const generateTurnSceneImages = async (turn: ScriptTurn, field: 'introScenes' | 'segmentScenes') => {
    ...
    const guide = studioCharacterGuideRef.current;
    setSceneLoading(prev => ({ ...prev, [turn.id]: true }));
  ...
  const clearTurnScenes = (turn: ScriptTurn, field: 'introScenes' | 'segmentScenes') => {
    // Keep studioCharacterGuideRef — other turns may still reference the same character.
  ...
  const combineVideoBlobs = async (
    ...
    // Get duration of each blob
    const getDuration = (blob: Blob): Promise<number> =>
      new Promise((res, rej) => {
        const v = document.createElement('video');
        v.src = URL.createObjectURL(blob);
        v.preload = 'metadata';
        v.onloadedmetadata = () => { URL.revokeObjectURL(v.src); res(isFinite(v.duration) ? v.duration : 0); };
        v.onerror = () => { URL.revokeObjectURL(v.src); rej(new Error('Video duration read nahi hua')); };
        setTimeout(() => rej(new Error('Duration read timeout')), 15_000);
      });

    onStatus('Duration read ho rahi hai…');
    const durations = await Promise.all(blobs.map(getDuration));
    const totalDuration = durations.reduce((a, b) => a + b, 0);

    // Canvas + AudioContext → merged MediaStream → MediaRecorder
    ...
    for (let i = 0; i < blobs.length; i++) {
      const partLabel = i === 0 && blobs.length === 3 ? 'Intro' : i === blobs.length - 2 ? 'Raw Clip' : 'Discussion';
      onStatus(`Part ${i + 1}/${blobs.length} (${partLabel}) play ho rahi hai — real-time…`);

      const blobUrl = URL.createObjectURL(blobs[i]);
      const startAcc = accumulatedSec;
      const partDur = durations[i];
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        const draw = () => {
          ...
          const progress = (startAcc + (video.currentTime || 0)) / totalDuration;
        ...
        video.onloadeddata = () => {
          try {
            audioSrc = actx.createMediaElementSource(video);
        ...
        video.onerror = () => {
          cleanup();
          reject(new Error(`Part ${i + 1} load nahi hua`));
        };

        setTimeout(() => { cleanup(); reject(new Error(`Part ${i + 1} timeout`)); }, (partDur + 30) * 1_000);
      });
    }
...
[
  ...
  const handlePodcastGenerate = useCallback(async (args: {
    ...
    // Persist podcast metadata so Export tab can render IntroFlow + clip trim
    // even after the generator steps are no longer visible.
    setPodcastSegments(args.segments);
    setPodcastTitle(args.podcastTitle);
    setPodcastHost(args.podcastHost);
    setPodcastGuests(args.podcastGuests);
      ...
      <div style={{ flex: 1, overflowY: 'auto', WebkitOverflowScrolling: 'touch', paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
        ...
        {tab === 'visual' && (
          <div style={{ padding: 12, paddingBottom: 'calc(96px + env(safe-area-inset-bottom, 0px))', display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            {visualSub === 'image' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                <div style={{ padding: '10px 12px', borderRadius: 10, background: 'rgba(96,165,250,0.07)', border: '1px solid rgba(96,165,250,0.2)', fontSize: 11, color: 'rgba(255,255,255,0.5)', lineHeight: 1.5 }}>
                  🖼️ Is turn ke dialogue ke hisaab se auto storyboard scenes — Intro wala same character reference + zoom + PiP. Prompt par click = copy.
                </div>
                {!activeTurn ? (
            ...
            {/* ── Intro settings section — storyboard scenes for Intro turn only
                (same flow as English Video's Intro Settings, not per-speaker
                images). Optional intro MP4 export lives in the Export tab. ── */}
            {activeSettingsSection === 'intro' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                <div style={{ padding: '10px 12px', borderRadius: 10, background: 'rgba(34,211,238,0.07)', border: '1px solid rgba(34,211,238,0.2)', fontSize: 11, color: 'rgba(255,255,255,0.5)', lineHeight: 1.5 }}>
                  🎬 Intro storyboard — timed scenes (MS Paint style), refresh ke baad bhi save rehti hain. Prompt par click = copy. Har scene replace/upload kar sakte ho.
                </div>

                {!introTurn ? (
                  <div style={{ padding: 12, borderRadius: 10, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', fontSize: 11, color: 'rgba(255,255,255,0.35)', textAlign: 'center' }}>
                    Script mein koi Intro turn nahi mila (speaker tag "Intro" chahiye)
                  </div>
                ) : (
                  <>
                    <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.45)', fontStyle: 'italic', lineHeight: 1.4, margin: 0 }}>
          ...
          <div style={{ padding: 14, paddingBottom: 'calc(96px + env(safe-area-inset-bottom, 0px))', display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            {script.length > 0 && (
              <TitleThumbComboCard
                ...
                topicName={sourceClips[0]?.title || podcastTitle || ''}
              />
            )}
...
export default PhoneConvoStudio2;
